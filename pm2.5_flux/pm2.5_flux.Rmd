...
class(narrows_date$date_new)

narrows_class <- as.POSIXct.Date(narrows_date$date_new)

class(narrows_class$date_new)

narrows_parse_date <- narrows_date %>% 
  mutate(obs_ymd = lubridate::ymd(date_new),
         obs_hour = lubridate::hms(date_new))

narrows_date$date_ymd <- as.Date(narrows_date$date_new)
narrows_date$date_hms <- format(narrows_date$date,"%H")
